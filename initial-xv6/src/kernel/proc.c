...
found:
  ...
  p->state = USED;
  p->atime = 0;
  // Initialize counter for readcount
  p->readcount = 0;

  // Initialize sig variables
  p->n = 0;
  p->alarmhandler = 0;
  p->astate = 0;
  p->aset = 0;
...
// Time to create two schedulers - 1) FCFS 2) MLFQ
#ifdef FCFS
void scheduler(void)
{
  struct proc *p;
  ...
  for (;;)
  {
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      if (to_run != 0 && p->ctime < to_run->ctime)
      {
  ...
  for (;;)
  {
    ...
    for(p=proc;p<&proc[NPROC];p++)
    {
      ...
      if(p->state==RUNNABLE)
      {
        ...
        else if(t->priority>p->priority)
        {
          t=p;
        }
        else if(t->priority==p->priority && t->wtime<p->wtime)
        {
          t=p;
        }
      }
...
void update_time()
{
  ...
  for (p = proc; p < &proc[NPROC]; p++)
  {
    ...
    #ifdef MLFQ
    else if(p->state==RUNNABLE && p->hastosleep==0)
    {
      p->wtime++;
      if(p->wtime>=30 && p->priority>0)
      {
        p->priority--;
        p->wtime=0;
        p->qtime=0;
